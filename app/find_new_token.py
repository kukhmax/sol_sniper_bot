...
async def calc_LP_locked_pct(
    ...
    try:
        actual_supply_lp = account_info.value.data.parsed['info']['supply']
        cprint(f"acctual_supply_lp: {actual_supply_lp}", "white", "on_black", attrs=["bold"])

        if lp_reserve == -69:
            burn_amt = "Failed"
            burn_pct = "Failed"
        else:
            # вычисляем процент сжигания LP
            lp_reserve = float(lp_reserve)
            actual_supply  = float(actual_supply_lp)
            burn_amt = lp_reserve - actual_supply
            cprint(f"burn_amt: {burn_amt}", "blue", "on_yellow", attrs=["bold"])

            burn_pct = burn_amt / lp_reserve * 100
        lp_locked_pct_ = str("%.2f".format(burn_pct))

    except Exception as e:
        cprint(f"Ошибка при проверски BURN TOKENS: {str(e)}", "red", attrs=["reverse", "blink"])
        pass
    
    return lp_locked_pct_

async def get_lp_locked_pct(token_address):
    url = f"https://api.rugcheck.xyz/v1/tokens/{token_address}/report"
    cprint(f"URL: {url}", "green")

    with requests.get(url) as data:
        rug_check = data.json()
        # cprint(f" rug_check: {rug_check}", "grey")
        try:
            lp_lock_pct = rug_check["markets"][0]['lp']['lpLockedPct']
            cprint(f"Token mint address: {token_address}, lp_lock_pct: {lp_lock_pct}", "black", "on_light_yellow")
            cprint(f"reserveSupply: {rug_check['markets'][0]['lp']['reserveSupply']}", "green", "on_dark_grey")
            cprint(f"currentSupply: {rug_check['markets'][0]['lp']['currentSupply']}", "green", "on_dark_grey")
            if rug_check["risks"]:
                for risk in rug_check["risks"]:
                    cprint(f"{risk['name']} : {risk['value']}", "blue", "on_white")
                    cprint(f"=== {risk['description']} ===", "green", "on_white")
                    cprint(f"Score: {risk['score']}", "blue", "on_white")
                    if risk['level'] == "danger":
                        cprint(f"==== {risk['level']} ====", "red", attrs=["reverse", "blink"])
                    elif risk['level'] == "warn":
                        cprint(f"==== {risk['level']} ====", "magenta", attrs=["reverse", "blink"])
                    print()
        except Exception as e:
            cprint(f"ERROR: {str(e)}", "red", attrs=["reverse", "blink"])
            lp_lock_pct = "failed"
...
async def get_tokens(signature: Signature, RaydiumLPV4: Pubkey) -> None:
    ...
    for instruction in filtered_instructions:
        ...
        rug_check_lp_locked_pct = await get_lp_locked_pct(mint)
        break
